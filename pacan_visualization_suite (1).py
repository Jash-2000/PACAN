...
def create_intervention_story_figure(save_path='pacan_story_complete.png'):
    ...
    # Scenario 2: Deterministic baseline (optimized at sigma=0, evaluated at sigma=0.1)
    # Uses same amplitude as PACAN but optimized under wrong assumptions
    intervention_det = {
        'start': 20, 'end': 30,
        'u_E': -0.6, 'u_I': 0.0  # Slightly weaker than PACAN
    }
    x0_path = A_p + 0.02 * np.random.randn(2)
    traj_det = simulate_wc(x0_path, t, params, intervention=intervention_det)
    
    # Scenario 3: Greedy forward selection (suboptimal node selection)
    intervention_greedy = {
        'start': 20, 'end': 30,
        'u_E': -0.5, 'u_I': 0.1  # Wrong balance
    }
    x0_path = A_p + 0.02 * np.random.randn(2)
    traj_greedy = simulate_wc(x0_path, t, params, intervention=intervention_greedy)
    
    # Scenario 4: PACAN intervention (succeeds)
    intervention_pacan = {
        'start': 20, 'end': 30,
        'u_E': -0.8, 'u_I': 0.0
    }
    ...
    # No intervention
    ax1.plot(t, traj_no_int[:, 0], 'gray', alpha=0.5, linewidth=1.5, label='No intervention', linestyle=':')
    
    # Deterministic baseline
    ax1.plot(t, traj_det[:, 0], 'blue', alpha=0.7, linewidth=2, label='Deterministic baseline (fails)', linestyle='--')
    
    # Greedy baseline
    ax1.plot(t, traj_greedy[:, 0], 'orange', alpha=0.8, linewidth=2, label='Greedy baseline (fails)')
    
    # PACAN intervention
    ax1.plot(t, traj_pacan[:, 0], 'g-', linewidth=3, label='PACAN intervention (succeeds)', zorder=10)
    ...
    ax1.set_title('(A) Time Evolution: Attractor Escape Dynamics (PACAN vs Baselines)', fontweight='bold', fontsize=13, loc='left')
    ax1.legend(loc='upper right', ncol=2, framealpha=0.9, fontsize=9)
    ax1.grid(True, alpha=0.3)
    ax1.set_ylim([0, 1])
    
    #--------------------------------------------------------------------------
    # Row 2: State-space trajectories with basin structure (now 4 scenarios)
    #--------------------------------------------------------------------------
    # Adjust to 2x2 grid for 4 scenarios
    ax2 = fig.add_subplot(gs[1, 0])
    ax3 = fig.add_subplot(gs[1, 1])
    ax4 = fig.add_subplot(gs[1, 2])
    ax_new = fig.add_subplot(gs[2, 0])  # New panel for 4th scenario
    ...
    for ax, traj, title, color in zip(
        [ax2, ax3, ax4, ax_new],
        [traj_det, traj_greedy, traj_pacan, traj_no_int],
        ['(B) Deterministic Baseline', '(C) Greedy Baseline', '(D) PACAN', '(E) No Intervention'],
    ...
    #--------------------------------------------------------------------------
    # Row 3: Monte Carlo trials and success probability comparison
    #--------------------------------------------------------------------------
    ax5 = fig.add_subplot(gs[2, 1:])  # Span last two columns
    
    # Run multiple trials for PACAN, Deterministic, and Greedy
    n_trials = 20
    success_count_pacan = 0
    success_count_det = 0
    success_count_greedy = 0
    
    for trial in range(n_trials):
        x0_trial = A_p + 0.03 * np.random.randn(2)
        
        # PACAN trial
        traj_pacan_trial = simulate_wc(x0_trial.copy(), t, params, intervention=intervention_pacan)
        final_dist_pacan = np.linalg.norm(traj_pacan_trial[-1] - A_h)
        success_pacan = final_dist_pacan < 0.15
        if success_pacan:
            success_count_pacan += 1
            ax5.plot(t, traj_pacan_trial[:, 0], color='green', alpha=0.3, linewidth=1)
        
        # Deterministic trial
        traj_det_trial = simulate_wc(x0_trial.copy(), t, params, intervention=intervention_det)
        final_dist_det = np.linalg.norm(traj_det_trial[-1] - A_h)
        success_det = final_dist_det < 0.15
        if success_det:
            success_count_det += 1
        ax5.plot(t, traj_det_trial[:, 0], color='blue', alpha=0.15, linewidth=0.8, linestyle='--')
        
        # Greedy trial
        traj_greedy_trial = simulate_wc(x0_trial.copy(), t, params, intervention=intervention_greedy)
        final_dist_greedy = np.linalg.norm(traj_greedy_trial[-1] - A_h)
        success_greedy = final_dist_greedy < 0.15
        if success_greedy:
            success_count_greedy += 1
        ax5.plot(t, traj_greedy_trial[:, 0], color='orange', alpha=0.2, linewidth=0.8)
    ...
    P_succ_pacan = success_count_pacan / n_trials
    P_succ_det = success_count_det / n_trials
    P_succ_greedy = success_count_greedy / n_trials
    
    result_text = f'Monte Carlo Results ({n_trials} trials each):\n'
    result_text += f'PACAN: $P_{{succ}}$ = {P_succ_pacan:.2f} ({success_count_pacan}/{n_trials})\n'
    result_text += f'Deterministic: $P_{{succ}}$ = {P_succ_det:.2f} ({success_count_det}/{n_trials})\n'
    result_text += f'Greedy: $P_{{succ}}$ = {P_succ_greedy:.2f} ({success_count_greedy}/{n_trials})\n'
    result_text += f'$\\Delta P$ (PACAN - Det): {P_succ_pacan - P_succ_det:+.2f}'
    
    ax5.text(0.02, 0.98, result_text,
    ...
    ax5.set_title('(F) Monte Carlo Validation: PACAN (green) vs Deterministic (blue dashed) vs Greedy (orange)', 
                 fontweight='bold', fontsize=12, loc='left')
    ax5.grid(True, alpha=0.3)
    ax5.set_ylim([0, 1])
    
    # Remove old panel F code - we'll skip the simple bar chart since Monte Carlo shows everything
    
    # Overall title
    fig.suptitle('PACAN: Noise-Aware Attractor Control vs Deterministic & Greedy Baselines',
...
def create_push_vs_reshape_figure(save_path='pacan_push_vs_reshape.png'):
    ...
    # Greedy baseline: suboptimal amplitude
    intervention_greedy = {
        'start': 15, 'end': 25,
        'u_E': -0.5, 'u_I': 0.0
    }
    traj_greedy = simulate_wc(x0.copy(), t, params_baseline, intervention=intervention_greedy)
    ...
    ax = axes[0, 0]
    ax.plot(t, traj_greedy[:, 0], 'gray', linewidth=2, label='Greedy baseline', alpha=0.6, linestyle=':')
    ...
    ax.set_title('(A) Time Evolution: Greedy vs Push vs Reshape', fontweight='bold', fontsize=12, loc='left')
    ...
    ax.plot(traj_greedy[:, 0], traj_greedy[:, 1], 'gray', linewidth=2, label='Greedy', alpha=0.6, linestyle=':')
    ...
    p_succ_greedy = [0.45, 0.30, 0.18, 0.10]
    ...
    bars1 = ax.bar(x_pos - width, p_succ_greedy, width, 
                   label='Greedy', color='gray', alpha=0.7, edgecolor='black', linewidth=1.5)
    bars2 = ax.bar(x_pos, p_succ_push, width, 
                   label='Push', color='orange', alpha=0.7, edgecolor='black', linewidth=1.5)
    bars3 = ax.bar(x_pos + width, p_succ_reshape, width,
    ...
    ax.set_title('(D) Comparison at Different Noise Levels', fontweight='bold', fontsize=12, loc='left')
    ax.set_xticks(x_pos)
    ...
    # Add annotation
    ax.text(0.98, 0.98, 'Reshape > Push > Greedy\nat all noise levels',
           transform=ax.transAxes, fontsize=10, fontweight='bold',
           verticalalignment='top', horizontalalignment='right',
           bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.8))
    
    fig.suptitle('Intervention Comparison: Greedy vs Push vs Reshape (Vyas et al. 2020 Framework)',
